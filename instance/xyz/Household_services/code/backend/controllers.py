...
def signup():
    if request.method == "POST":
        ...
        new_usr = User(email=uname, password=pwd, full_name=fname, address=address, pin_code=pin, phone_no=phone)
...
@app.route("/pro/search/<name>", methods=["GET","POST"])
def pro_search(name):
    if request.method=="POST":
        search_txt= request.form.get("search_txt")
        by_service= search_by_service(search_txt)
        by_professionals= search_by_professional(search_txt)
        by_users=search_by_user(search_txt)
        by_service_req=search_by_service_req(search_txt)
        if by_service or by_professionals or by_users or by_service_req :
            return render_template("prodash.html", name=name, service=by_service,professional=by_professionals, user=by_users, service_request=by_service_req)
       

    return redirect(url_for("pro_dashboard",name=name))
...
@app.route('/accept_request/<id>/<name>', methods=['GET', 'POST'])
def accept_request(id,name):
...
@app.route("/close_service/<int:sid>/<name>", methods=["GET", "POST"])
def close_ticket(sid, name):
    if request.method == "POST":
        rating = request.form.get("rating")
        service_request = ServiceRequest.query.filter_by(s_id=sid, user_id=name).filter(or_(ServiceRequest.status == "Pending", ServiceRequest.status == "Approved")).first()
        if service_request: 
    ...
    if not service or not user:
        return "Service or User not found", 404
...
def search_by_user(search_txt):
    ...
    return usr
def search_by_service_req(search_txt):
    if not search_txt:
        return []
    usr = ServiceRequest.query.filter(ServiceRequest.date.ilike(f'%{search_txt}%')).all()
    return usr
